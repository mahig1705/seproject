...
export default function BillsPage() {
  ...
  const fetchBills = async () => {
    try {
      ...
      const res = await apiService.getBills({ userId: user._id });
  ...
  useEffect(() => {
  fetchBills();
  if (user?.role !== UserRole.RESIDENT) fetchUsers();
}, []);
  ...
  const getTotalStats = () => {
    ...
    const pendingAmount = bills.filter(b => b.status === PaymentStatus.PENDING).reduce((sum, b) => sum + b.amount, 0);
                ...
                <TableBody>
                  {filteredBills.map(bill => (
                    <TableRow key={bill._id}>
                      ...
                      <TableCell>
                        {user?.role === UserRole.RESIDENT && bill.status?.toUpperCase() === 'PENDING' && (
